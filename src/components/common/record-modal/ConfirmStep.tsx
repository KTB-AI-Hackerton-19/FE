...
function ConfirmStep({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex min-h-0 flex-1 flex-col">
      {/*
        버튼만 아래에 고정하고 나머지는 이 안에서 스크롤한다.
        제목·탭까지 넣어 두어야 어디에 마우스를 올려도 스크롤된다.
        좌우로 넓혀 스크롤바가 칸에 붙지 않게 하고, 그만큼 다시 안쪽 여백을 준다.
        아래 여백은 버튼 위를 덮는 그라데이션 높이만큼 — 없으면 마지막 칸이 가려진 채 남는다.
      */}
      <div className="-mx-[19px] min-h-0 flex-1 overflow-auto px-[19px] pb-7 sm:-mx-[34px] sm:px-[34px]">
      ...
      {/* 스크롤 영역 끝을 옅게 덮어 내용이 버튼 뒤로 사라지는 것처럼 보이게 한다. */}
      <div className="relative shrink-0 pt-4 before:pointer-events-none before:absolute before:inset-x-0 before:-top-7 before:h-7 before:bg-gradient-to-t before:from-[#fffdfa] before:to-transparent before:content-['']">
        {firstError ? <p className="mb-2 text-[10px] text-coral-dark">{firstError}</p> : null}
